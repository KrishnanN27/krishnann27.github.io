...
<style>
  ...
  /* Custom Collapsible Style for Work Experience */
.work-details {
...
.work-summary {
  font-weight: bold;
  color: var(--primary-color); /* Primary color for work titles */
  cursor: pointer;
  padding: 10px; /* Add padding for summary block */
  background-color: transparent; /* Default no background */
  display: flex;
  align-items: center; /* Vertically align arrow and text */
  transition: background-color 0.3s ease; /* Transition for background color */
  border-radius: 0; /* Remove the corner rounding effect */
}
...
.work-details[open] .work-summary {
  background-color: var(--summary-open-bg); /* Background color when expanded */
}

.work-details[open] .work-summary::before {
  transform: rotate(90deg); /* Rotate to down-pointing arrow */
}

.work-link i {
  margin-left: 10px;
  color: var(--link-color); /* Match link color to link color variable */
}

.work-details div {
  margin-top: 10px;
  padding-left: 15px; /* Indent content slightly */
  color: var(--primary-color); /* Ensure all text is primary color */
}
...
.work-details p i {
  ...
  color: var(--link-color); /* Use the link color for icons */
}
